                                ...
                                <div class="col-sm-12">
                                    <table id="add-row" class="display table table-striped table-hover dataTable"
                                        ...
                                        <tbody>
                                            ...
                                            @forelse ($file as $index => $row)
                                                <tr role="row" class="odd">

                                                    <td>{{ $index + $file->firstItem() }}</td>
                                                    <td>{{ $row->nama }}</td>
                                                    ...
                                                    <td>
                                                        <div class="form-button-action">
                                                            ...
                                                            <a href="{{ route('detail-file-admin', $row->id) }}"
                                                                class="btn btn-primary btn-sm mt-2 mr-2">Lihat/Download</a>
                                                            <a href="{{ route('file.edit', $row->id) }}"
                                                                class="btn btn-warning btn-sm mt-2">Edit</a>
                                                            <form action="{{ route('file.destroy', $row->id) }}"
                                                                method="POST"
                                                                class="
                                                            d-inline">
                                                                @csrf
