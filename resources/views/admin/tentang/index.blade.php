        ...
        <div class="panel-header bg-primary-gradient">
            <div class="page-inner py-5">
                <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row">
                    <div>
                        <h2 class="text-white pb-2 fw-bold">Data Tentang Sekolah</h2>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex align-items-center">
                        ...
                        <a href="{{ route('admintentang.create') }}" class="btn btn-primary btn-sm ml-auto"><i
                                ...
                                <div class="col-sm-12">
                                    <table id="add-row" class="display table table-striped table-hover dataTable"
                                        ...
                                        <tbody>
                                            ...
                                            @forelse ($tentang as $index => $row)
                                                <tr role="row" class="odd">

                                                    <td>{{ $index + $tentang->firstItem() }}</td>
                                                    <td>{{ $row->judul }}</td>
                                                    {{--
                                                    <td>{{ $data = Str::of($row->deskripsi)->substr(0, 50) }}
                                                    </td> --}}

                                                    <td><img src="{{ asset('uploads/' . $row->gambar_tentang) }}"
                                                            width="50" alt=""></td>


                                                    <td>
                                                        <div class="form-button-action">
                                                            <a href="{{ route('detail-tentang-admin', $row->id) }}"
                                                                class="btn btn-primary btn-sm mt-2 mr-2">Lihat</a>

                                                            <a href="{{ route('admintentang.edit', $row->id) }}"
                                                                class="btn btn-warning btn-sm mt-2">Edit</a>
                                                            <form action="{{ route('admintentang.destroy', $row->id) }}"
                                                                method="POST"
                                                                class="
                                                        d-inline">
                                                                @csrf
