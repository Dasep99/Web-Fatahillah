            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex align-items-center">
                        ...
                        <a href="{{ route('sambutans.create') }}" class="btn btn-primary btn-sm ml-auto"><i
                                    ...
                                    <table id="add-row" class="display table table-striped table-hover dataTable"
                                        ...
                                        <thead>
                                            <tr role="row">
                                                ...
                                                <th style="width: 126.281px;" class="sorting" tabindex="0"
                                                    aria-controls="add-row" rowspan="1" colspan="1"
                                                    aria-label="Action: activate to sort column ascending">Deskripsi</th>
                                        ...
                                        <tbody>
                                            ...
                                            @forelse ($sambutan as $index => $row)
                                                <tr role="row" class="odd">

                                                    <td>{{ $index + $sambutan->firstItem() }}</td>
                                                    <td>{{ $row->judul }}</td>

                                                    <td>{!! $datas = Str::of($row->deskripsi)->substr(0, 50) !!}
                                                    </td>

                                                    <td><img src="{{ asset('uploads/' . $row->gambar_sambutan) }}"
                                                            width="50" alt=""></td>

                                                    <td>
                                                        <div class="form-button-action">
                                                            <a href="{{ route('detail-sambutan-admin', $row->id) }}"
                                                                class="btn btn-primary btn-sm mt-2 mr-2">Lihat</a>

                                                            <a href="{{ route('sambutans.edit', $row->id) }}"
                                                                class="btn btn-warning btn-sm mt-2">Edit</a>
                                                            <form action="{{ route('sambutans.destroy', $row->id) }}"
                                                                method="POST"
                                                                class="
                                                            d-inline">
                                                                @csrf
