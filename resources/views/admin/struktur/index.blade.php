            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex align-items-center">
                        ...
                        <a href="{{ route('strukturadmin.create') }}" class="btn btn-primary btn-sm ml-auto"><i
                                ...
                                <div class="col-sm-12">
                                    <table id="add-row" class="display table table-striped table-hover dataTable"
                                        ...
                                        <tbody>
                                            ...
                                            @forelse ($struktur as $index => $row)
                                                <tr role="row" class="odd">

                                                    <td>{{ $index + $struktur->firstItem() }}</td>
                                                    <td>{{ $row->judul }}</td>
                                                    {{-- <td>{!! $data = Str::of($row->deskripsi)->substr(0, 50) !!}</td> --}}

                                                    <td><img src="{{ asset('uploads/' . $row->gambar_struktur) }}"
                                                            width="50" alt=""></td>

                                                    <td>
                                                        <div class="form-button-action">
                                                            <a href="{{ route('detail-struktur-admin', $row->id) }}"
                                                                class="btn btn-primary btn-sm mt-2 mr-2">Lihat</a>

                                                            <a href="{{ route('strukturadmin.edit', $row->id) }}"
                                                                class="btn btn-warning btn-sm mt-2">Edit</a>
                                                            <form action="{{ route('strukturadmin.destroy', $row->id) }}"
                                                                method="POST"
                                                                class="
                                                            d-inline">
                                                                @csrf
